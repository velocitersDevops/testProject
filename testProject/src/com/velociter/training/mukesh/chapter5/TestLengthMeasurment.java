...
class LengthMeasurment 
{
	...
	// Constructors
	  ...
	  public LengthMeasurment(int meters, int centimeters, int millimeters) 					//three argument constructor pass as integer type
	  {
			 ...
			 this((int)(meters*MILLIMETER_PER_METER + centimeters*MILLIMETER_PER_CENTIMETER + millimeters));
 ...
 public class TestLengthMeasurment
{
	public static void main(String[] args) 
	{
		LengthMeasurment[] lengths = new LengthMeasurment[4];
			double firstValue=274.65;
			int secondValue =274;
			if(firstValue>0)
			lengths[0] = new LengthMeasurment(firstValue);			//double value  argument
			else {
				System.out.println("provide only positive number");
				System.exit(0);
			}
			if(secondValue>0)
			lengths[1] = new LengthMeasurment(secondValue);			//integer value argument
			else {
				System.out.println("provide only positive  number");
				System.exit(0);
			}
